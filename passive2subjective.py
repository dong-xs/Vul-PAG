# encoding:utf-8
'''
    此py文件用于将被动语态的句子转换为主动语态，主要借鉴于EXTRACTOR中的代码方式.
    主要是pass2act这个函数：
    输入：一个句子；
    输出：一个重构后的句子。
'''
...
def nouninv(noun):    #函数的作用是将名词的主语与宾语形式进行转换
    n = noun.lower()
    if n in noundict:
        return noundict[n]
    return noun
...
def pass2act(doc, rec=False):
    ...
    for sent in parse.sents:
        ...
        # invert nouns:
        agent = nouninv(agent)     #代词转换
        subjpass = nouninv(subjpass)    #主语转换
...
sentence='This product is subjected to XSS.'

prev = ''
acts = ''

print(pass2act(sentence))
...
def argmax(vec):
    _, idx = torch.max(vec, 1)  # 返回一个tensor在每行上的最大值，会返回两个tensor，第一个是每行的最大值，第二个是每行最大值的索引位置
    # torch.max(input,dim)：
    ...
    # 综上所述，该函数的意义就是返回一个输入m*n维tensor在每一行上最大值的索引值，输出维度为1行m列。


def prepare_sequence(seq, to_ix):
    # seq是一个输入的序列，to_ix表示将原来的句子转换为一个
    # to_ix是一个标签对序列
    idxs = [to_ix[w] for w in seq]  # 这句代码的意思是将序列中的每个词转换到对应的的标签序列上，返回模式为一个列表，
    # 即序列中每个元素在to_ix列表中的索引位置，返回维度为与seq等长的一个列表
...
# 训练阶段:原始代码
# train_data=[("the wall street journal reported today that apple corporation made money".split(),"B I I I O O O B I O O".split()),
